...
import type { EmployeeSummaryCardsProps } from "../types";
...
export function EmployeeSummaryCards({ cards }: EmployeeSummaryCardsProps) {
  return (
    <section className="grid gap-5 md:grid-cols-2 xl:grid-cols-4">
      {cards.map((card) => {
        ...
        return (
          <Card key={card.label} className="rounded-[18px] border-border bg-card shadow-xl shadow-black/5 dark:border-white/10 dark:bg-[#141417] dark:shadow-black/20">
            <CardContent className="flex items-center gap-4 p-5">
              ...
              <div>
                ...
                <p className="mt-1 text-3xl font-bold text-foreground dark:text-white">{card.value}</p>
              </div>
            </CardContent>
          </Card>
        );
